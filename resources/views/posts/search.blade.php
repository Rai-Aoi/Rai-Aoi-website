        ...
        <table class="w-full text-left text-gray-600 dark:text-gray-400">
            <thead class="font-mono text-lg text-gray-700 bg-gray-200 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    @canany(['viewForStudentAffair', 'viewForAdmin'], \App\Models\Post::class)
                        <th scope="col" class="py-3 px-6">ชื่อผู้ใช้</th>
                    @endcanany
            ...
            <tbody class="m-2">
            @can(['viewForStaff'], \App\Models\Post::class)
                @foreach($posts as $post)
                    @foreach($post->processes as $process)
                    @if($process->name != 'รอรับเรื่อง' & $process->name != 'ไม่อนุมัติ') 
                    <tr class="border-t">
                    <td>
            ...
            @foreach($posts as $post)
                <tr class="border-t">
                    ...
                    @foreach($post->processes as $process)
                            @if($process->name == 'รอรับเรื่อง') 
                            <a href="{{ route('processes.show', ['process' => $process->name]) }}">
                                <p class="border-2 border-blue-300 text-blue-400 text-xs inline-flex items-center px-2.5 py-0.5 rounded mr-2">
                                    {{ $process->name }}
                                </p>
                            </a>
                            @elseif($process->name == 'ดำเนินการ') 
                            <a href="{{ route('processes.show', ['process' => $process->name]) }}">
                                <p class="border-2 border-yellow-300 text-yellow-400 text-xs inline-flex items-center px-2.5 py-0.5 rounded mr-2">
                                    {{ $process->name }}
                                </p>
                            </a>
                            @elseif($process->name == 'เสร็จสิ้น') 
                            <a href="{{ route('processes.show', ['process' => $process->name]) }}">
                                <p class="border-2 border-[#B3BA1E] text-[#B3BA1E] text-xs  inline-flex items-center px-2.5 py-0.5 rounded mr-2">
                                    {{ $process->name }}
                                </p>
                            </a>
                            @elseif($process->name == 'ไม่อนุมัติ') 
                            <a href="{{ route('processes.show', ['process' => $process->name]) }}">
                                <p class="border-2 border-red-300 text-red-400 text-xs inline-flex items-center px-2.5 py-0.5 rounded mr-2">
                                    {{ $process->name }}
                                </p>
                            </a>
                            @endif
                    ...
                    @canany(['viewForUser', 'viewForStudentAffair', 'viewForAdmin'], \App\Models\Post::class)
                    <td>
                        @can('update', $post)
                        <div>
                            <a class="font-mono text-gray-600 text-sm p-2 m-3 ml-0.5 rounded bg-gray-200 hover:bg-gray-300" href="{{ route('posts.edit', ['post' => $post->id]) }}">
